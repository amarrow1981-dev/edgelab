...
| get_successful_param_directions — proper signed direction | ✅ | S30. Rebuilt with JOIN + signed delta. Previous was meaningless. |
| DPOL reads from candidate log | ✅ | S30. DPOLManager accepts db. Directional bias in _generate_candidates. |
| S30 DPOL run — first with logging active | ⏳ | In progress at S30 close. Result to log at S31 start. |
| S31 DPOL run — first with navigation active | 🔲 | S31 queue item 2. After S30 result logged. |
...
| Three-pass rerun after navigated DPOL | 🔲 | After S31 navigated run assessed. |
...
| Gary nn activated in weekly pipeline | 🔲 | S31 queue item 3. Pass db to GaryBrain in gary.py + predict.py. |
...
| Weather cache completion | ⏳ | 123,926/132,685. Check S31. |
...
| Item | Status | Notes |
|------|--------|-------|
| Gary nn activated in weekly pipeline | 🔲 | S31 queue item 3. db not passed in gary.py/predict.py yet. |
| Gary upset analysis JSON injection | 🔲 | S31 queue item 4. HTML card structure built, text is placeholder. |
...
| HTML generator — Gary upset analysis injection | 🔲 | S31 queue item 4. Companion JSON file. |
| Acca filter rebuild | 🔲 | S31 queue item 5. result/safety/value not meaningfully distinct. |
| Automated prediction pipeline (one command) | 🔲 | Small build |
...
| DataBot | ✅ | |
| Weather cache | ⏳ | 123,926/132,685 (93.4%). Check S31. |
...
| edgelab_html_generator.py | ✅ | S30. NEW. Full predictions HTML generator. |
...
| Full weekly commands documented | ✅ | In briefing + state — now 7 steps (HTML generator added S30) |
| HTML generator in weekly workflow | ✅ | S30. Step 7. Replaces manual HTML build. |
| Automated prediction pipeline (one command) | 🔲 | Small build |
...
| Public predictions HTML | ✅ | S30. Fully automated. iOS confirmed working. |
| Predictions HTML on Netlify | ✅ | S30. spectacular-licorice-3d5119.netlify.app |
| Rename Netlify predictions site | 🔲 | S31 queue item 7. → gary-picks.netlify.app |
...
| Date labelling — fixtures shown a day early | ✅ | CLOSED S30. Was manual HTML error, not code bug. |
| Public HTML — iOS tab/search broken | ✅ | Fixed S28. Confirmed working S30 via Netlify URL. |
| DPOL strips w_score_margin on each run | ✅ | Root cause fixed S29/S30. DB + directional bias now prevents this. |
| Nearest-neighbour query full table scan | ⏳ | Acceptable at 131k rows. Flag for refactor. |
| Acca filter — result/safety/value not meaningfully distinct | 🔲 | S31 queue item 5. All pull from same pool. |
| Gary upset analysis in HTML is generic placeholder | 🔲 | S31 queue item 4. Wire companion JSON. |
| Gary pattern memory not in weekly pipeline | 🔲 | S31 queue item 3. db=None in gary.py/predict.py. |
...
| get_successful_param_directions — proper signed direction | ✅ | S30. Rebuilt with JOIN. Previous was meaningless. |
| DPOL reads from candidate log | ✅ | S30. Directional bias in _generate_candidates. |
| S30 DPOL run — populates candidate log | ⏳ | In progress at S30 close. |
| S31 DPOL run — first with navigation active | 🔲 | S31 queue item 2. The real test. |
| Stage 2 draw rate strategy (25-27% band) | 🅿 | Trigger: three-pass proven on draws. |
| Core param + signal combination search (4th coordinate) | 🅿 | PARKED S29. Design into three-pass rebuild. |
...
Source: Sessions 1-30 + full Anthropic export (all 29 conversations, 10/04/2026)
Last updated: Session 30 — 11/04/2026
Next update: Session 31 close (unprompted — as files)
